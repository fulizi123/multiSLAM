...
namespace LL_SLAM
{
    ...
    public:
        ...
        int SearchByReferenceKFBruteForce(KeyFrame *pKF, Frame *pF);

        int SearchByProject(std::vector<MapPoint*> &vpLocalMP, Frame *pF);

        int SearchByProjectMultiCamera(std::vector<MapPoint*> &vpLocalMP, Frame *pF, int nMaxMatchNum = -1);
        void SearchByProjectMultiThread(vector<MapPoint*> *pvpLocalMP, Frame *pF, int cam_i, int minLevel, int maxLevel, int w, int h, int nMaxMatchNum,
                                        const Eigen::Matrix3f &Rcw, const Eigen::Vector3f &tcw, const Eigen::Matrix3f &K_cam,
                                        vector<int> *pvnMatches);


        int SearchByEpipolarGeometryBetweenImage(KeyFrame *pKF2, int cam_2, KeyFrame *pKF1, int cam_1, int nMinKF, int nMaxKF);

        int SearchByEpipolarGeometryMultiCamera(vector<KeyFrame *> vpLocalKF, int nMinKF, int nMaxKF);
        int SearchByEpipolarGeometryMultiCameraMultiThread(vector<KeyFrame *> vpLocalKF, int cam_i, int nMinKF, int nMaxKF);
